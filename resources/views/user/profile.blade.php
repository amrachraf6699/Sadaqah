...
<div class="container mx-auto mt-8">
    ...
    <!-- User's withdrawals section -->
<div class="mt-8">
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-3xl underline font-semibold">Your Withdrawals</h2>
        <a href="{{ route('user.withdraw.create') }}" class="border border-transparent rounded-full font-semibold tracking-wide text-lg md:text-sm p-3 focus:outline-none focus:shadow-outline bg-yellow-500 text-white hover:bg-yellow-200 transition-all duration-300 ease-in-out flex items-center justify-center w-12 h-12">
            <i class='bx bx-wallet text-2xl'></i>
        </a>
    </div>
    @if($withdraws->isEmpty())
        <p class="text-gray-600">You have not made any withdrawals yet.</p>
    @else
            ...
            <table class="min-w-full bg-white shadow-lg rounded-lg overflow-hidden">
                <thead class="bg-gray-200">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-sm font-medium text-gray-700 uppercase tracking-wider">Payment Method</th>
                        <th class="px-6 py-3 text-left text-sm font-medium text-gray-700 uppercase tracking-wider">Amount</th>
                        <th class="px-6 py-3 text-left text-sm font-medium text-gray-700 uppercase tracking-wider">Date</th>
                        <th class="px-6 py-3 text-left text-sm font-medium text-gray-700 uppercase tracking-wider">Notes</th>
                    </tr>
                </thead>
                        ...
                        <tr class="hover:bg-gray-100 transition-colors duration-200">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                @switch($withdraw->status)
                                    @case('pending')
                                        <i class='bx bx-time text-2xl text-yellow-500' title="Pending"></i>
                                        @break
                                    @case('processing')
                                    ...
                                    @case('paid')
                                        ...
                                        @break
                                    @case('regected')
                                        <i class='bx bx-x text-2xl text-red-500' title="Rejected"></i>
                                        @break
                                    @default
                                        <i class='bx bx-question-mark text-2xl text-gray-500' title="Unknown"></i>
                                @endswitch
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $withdraw->paymentMethod->name }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">${{ number_format($withdraw->amount, 2) }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $withdraw->created_at->format('F d, Y') }} <span class="text-xs">{{ $withdraw->created_at->format('h:i A') }}</span></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $withdraw->notes }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
